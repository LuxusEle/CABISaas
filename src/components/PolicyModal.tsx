...
import { X, ChevronRight, FileText, Shield } from 'lucide-react';
...
const parseMarkdown = (content: string): React.ReactNode => {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith('|') && line.endsWith('|')) {
      ...
      const table = (
        <table key={elements.length} className="w-full border-collapse my-4 text-sm">
          <thead>
            <tr className="bg-slate-100 dark:bg-slate-800">
              {tableRows[0]?.map((cell, j) => (
                <th key={j} className="border border-slate-300 dark:border-slate-600 px-3 py-2 text-left font-bold text-slate-900 dark:text-white">{cell}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {tableRows.slice(1).map((row, ri) => (
              <tr key={ri}>
                {row.map((cell, j) => (
                  <td key={j} className="border border-slate-300 dark:border-slate-600 px-3 py-2 text-slate-600 dark:text-slate-300">{cell}</td>
                ))}
              </tr>
            ))}
          </tbody>
    ...
    if (line.trim() === '' || line.startsWith('>')) {
      ...
      flushParagraph();
      if (line.startsWith('>')) {
        elements.push(<blockquote key={elements.length} className="border-l-4 border-orange-500 pl-4 italic text-slate-600 dark:text-slate-300 my-4">{line.slice(1).trim()}</blockquote>);
      }
      continue;
    }
...
`;
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fade-in">
      ...
      `}</style>
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] flex flex-col animate-modal-pop border border-slate-200 dark:border-slate-700">
        {/* Header */}
        ...
        <div className="flex flex-1 overflow-hidden">
          {/* Sidebar */}
          <aside className="hidden md:block w-48 border-r border-slate-200 dark:border-slate-700 overflow-y-auto p-2">
        ...
        <div className="p-4 border-t border-slate-200 dark:border-slate-700 shrink-0 flex justify-end">
          <button
            onClick={onClose}
            className="px-6 py-2 bg-gradient-to-r from-orange-500 to-orange-500 hover:from-orange-600 hover:to-orange-600 text-white font-bold rounded-xl transition-all"
          >
            Close
          </button>
        </div>
