...
export const WallLimitsModal = forwardRef<any, WallLimitsModalProps>(({
  ...
  const handleUpdateIslandSetting = (key: string, value: number | boolean) => {
    ...
    return (
        ...
        <div className="absolute inset-0 p-2 sm:p-4 flex flex-col">
          <div className="flex-1 flex gap-2 min-h-0">
            <div className="flex-1 relative bg-white dark:bg-slate-900 rounded-2xl border-2 border-slate-200 dark:border-slate-800 shadow-2xl overflow-hidden">
              <svg viewBox={`0 0 ${viewW} ${viewH}`} className="w-full h-full" style={{ fontSize: '8px' }}>
                <defs>
                ...
                <text x={offX + islLeft + islW / 2} y={offY + islTop + islH / 2}
            ...
            <div className="flex flex-col items-center bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-lg py-2 sm:py-3 w-10 shrink-0">
              ...
              <div className="flex flex-col items-center gap-0.5">
                <input type="number" value={dirtyInputs.posZ ?? Math.round(isl.posZ ?? 1500)}
                  ...
                  className="w-12 bg-transparent text-sm font-black text-amber-500 font-mono text-right outline-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none border-b border-transparent focus:border-amber-500 transition-all" />
                <span className="text-[10px] font-black text-amber-500 font-mono">mm</span>
              </div>
            </div>
          </div>
              ...
              <div className="flex items-center justify-between mb-1">
                ...
                <div className="flex items-center gap-1">
                  <input type="number" value={dirtyInputs.posX ?? Math.round(isl.posX ?? 1800)}
                    ...
                    onKeyDown={(e) => { if (e.key === 'Enter') (e.target as HTMLInputElement).blur(); }}
                    className="w-16 bg-transparent text-sm font-black text-amber-500 font-mono text-right outline-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none border-b border-transparent focus:border-amber-500 transition-all" />
                  <span className="text-xs font-black text-amber-500 font-mono">mm</span>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between mb-1">
                ...
                <button
                  ...
                  className={`relative w-10 h-5 rounded-full transition-all shrink-0 ${
                    isl.hasSeating ? 'bg-emerald-500' : 'bg-slate-300 dark:bg-slate-700'
                  }`}
                >
                  <div className={`absolute top-0.5 w-4 h-4 bg-white rounded-full shadow-md transition-all ${
                    isl.hasSeating ? 'left-5.5' : 'left-0.5'
                  }`} />
                </button>
              </div>
              {isl.hasSeating && (
