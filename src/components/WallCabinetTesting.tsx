      ...
      {showDoors && actualNumDoors > 0 && Array.from({ length: actualNumDoors }).map((_, i) => {
        ...
        return (
          <group key={`door-${i}`}>
            ...
            {shouldShow('door') && (
              <mesh position={[doorX + handleXOffset + getOffset('door', i)[0], -doorHeight / 2 + 50 + getOffset('door', i)[1], depth / 2 + doorMaterialThickness + 5 + getOffset('door', i)[2]]} rotation={[0, 0, Math.PI / 2]} castShadow>
...
export const exportWallCabinetDXF = async (settings: TestingSettings, zip: JSZip) => {
  const {
    ...
    showBackPanel, showDoors, showShelves, numShelves, hingeDiameter, hingeHorizontalOffset, hingeVerticalOffset,
    nailHoleDiameter
  } = settings;
