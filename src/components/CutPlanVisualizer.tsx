                ...
                {(() => {
                  if (cncData && cncData.cutouts && cncData.cutouts.length > 0) {
                     ...
                     if (isNewFormat) {
                       ...
                       const uMaxCT = !!(vMaxNotches.find((n: any) => Math.abs(n.uMax - uMax) < tol) || uMaxNotches.find((n: any) => Math.abs(n.vMax - vMax) < tol));
                       ...
                       const uMinCB = !!(vMinNotches.find((n: any) => Math.abs(n.uMin - uMin) < tol) || uMinNotches.find((n: any) => Math.abs(n.vMin - vMin) < tol));
                       ...
                       if (!uMinCB) panelPoints.push({ x: uMin, y: vMin });
                        ...
                        panelPoints.push({x: -cncW/2, y: -cncH/2});
                        panelPoints.push({x: cncW/2, y: -cncH/2});
                        const sorted = [...notches].sort((a: any, b: any) => a.y - b.y);
                        sorted.forEach((c: any) => {
                          panelPoints.push({x: cncW/2, y: c.y - cncH/2});
                          panelPoints.push({x: c.x - cncW/2, y: c.y - cncH/2});
                          panelPoints.push({x: c.x - cncW/2, y: c.y + c.h - cncH/2});
                          if (c.y + c.h < cncH - 0.1) panelPoints.push({x: cncW/2, y: c.y + c.h - cncH/2});
                        });
                     ...
                     // Convert to SVG points (transform from center-relative to top-left relative)
                     const mappedPoints = panelPoints.map(p => {
                        let px = p.x + cncW / 2;
                        let py = p.y + cncH / 2;
                        if (cncData.mirrorX) px = cncW - px;
