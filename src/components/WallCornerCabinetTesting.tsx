    ...
    <group position={[width / 2, innerHeight / 2, depth / 2]}>
      ...
      {shouldShow('topPanel') && (
        <mesh position={[0 + getOffset('topPanel')[0], innerHeight / 2 - panelThickness / 2 + getOffset('topPanel')[1], 0 + getOffset('topPanel')[2]]} castShadow receiveShadow visible={!skeletonView}>
          ...
          <meshStandardMaterial color={settings.isStudio && settings.woodTexture ? '#ffffff' : getPanelColor('topPanel')} map={settings.isStudio ? settings.woodTexture : undefined} roughness={0.4} metalness={0} side={THREE.DoubleSide} transparent={settings.opacity < 1} opacity={settings.opacity} />
        </mesh>
      )}
