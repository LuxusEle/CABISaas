...
export const exportWallCornerCabinetDXF = async (settings: TestingSettings, zip: JSZip | null, dataCollector?: (data: any) => void) => {
  const {
    ...
    nailHoleDiameter, shelfHoleDiameter, hingeDiameter, hingeHorizontalOffset, hingeVerticalOffset, wallBottomRecess, showNailHoles
  } = settings;
  ...
  if (showBackStretchers && !isLShort) {
    const zBack = -depth / 2 + panelThickness / 2;
    const topStretcherYTop = leftH / 2;
    const bottomStretcherYTop = -leftH / 2 + wallBottomRecess + 100;
    const zOffsetL = isLShort ? columnDepth / 2 : 0;
    
    [topStretcherYTop - 25, topStretcherYTop - 80, bottomStretcherYTop - 25, bottomStretcherYTop - 80].forEach(y => {
      const z = zBack - zOffsetL;
      leftHoles.push({ y, z: shouldMirrorL ? -z : z, r: nailHoleDiameter / 2 });
    });
  }
  ...
  if (isLShort) {
    ...
    calculateNailHolePositions(leftH).forEach(offset => {
      leftHoles.push({ y: offset, z: shouldMirrorL ? -zAttach : zAttach, r: nailHoleDiameter / 2 });
    });
  }
  ...
  if (showShelves && numShelves > 0) {
    ...
    for (let i = 0; i < numShelves; i++) {
      ...
      const zOffsetL = isLShort ? columnDepth / 2 : 0;
      if (isLShort) {
        const z1 = (depth / 2 - 50) - zOffsetL;
        const z2 = (-depth / 2 + columnDepth + 50) - zOffsetL;
        leftHoles.push({ y: holeY, z: shouldMirrorL ? -z1 : z1, r: shelfHoleDiameter / 2 });
        leftHoles.push({ y: holeY, z: shouldMirrorL ? -z2 : z2, r: shelfHoleDiameter / 2 });
      } else {
        const shelfHoleOffsets = calculateNailHolePositions(shelfLengthFull);
        [shelfHoleOffsets[0], shelfHoleOffsets[shelfHoleOffsets.length - 1]].forEach(offset => {
          const z = (zCenterShelfFull + offset) - zOffsetL;
          leftHoles.push({ y: holeY, z: shouldMirrorL ? -z : z, r: shelfHoleDiameter / 2 });
        });
      }
    }
  }
  const leftGrooveX = shouldMirrorL ? leftW - panelThickness - (backPanelThickness + 2) : panelThickness;
  const leftGroove = !isLShort ? { x: leftGrooveX, y: 0, w: backPanelThickness + 2, h: leftH } : undefined;
  ...
  if (showBackStretchers && !isRShort) {
    const zBack = -depth / 2 + panelThickness / 2;
    const topStretcherYTop = rightH / 2;
    const bottomStretcherYTop = -rightH / 2 + wallBottomRecess + 100;
    const zOffsetR = isRShort ? columnDepth / 2 : 0;
    
    [topStretcherYTop - 25, topStretcherYTop - 80, bottomStretcherYTop - 25, bottomStretcherYTop - 80].forEach(y => {
      const z = zBack - zOffsetR;
      rightHoles.push({ y, z, r: nailHoleDiameter / 2 });
    });
  }
  ...
  const rightGroove = !isRShort ? { x: panelThickness, y: 0, w: backPanelThickness + 2, h: rightH } : undefined;
  ...
  const shouldMirrorH = blindCornerSide === 'left';
  ...
  const tGrooveX_pre = (enableColumn && blindCornerSide === 'left') ? tGrooveX_base + columnWidth : tGrooveX_base;
  const tGrooveX = (width - tGrooveX_pre - tGrooveW);

  const mirroredTopHoles = topHoles.map(h => ({ ...h, z: -h.z }));
  const mirroredTopNotches = topNotches.map(n => ({ 
    ...n, 
    u: -n.u, 
    side: n.side === 'uMin' ? 'uMax' : (n.side === 'uMax' ? 'uMin' : n.side) 
  }));

  addPanelToZip('Top_Panel', width, depth, mirroredTopHoles, { x: tGrooveX, y: depth - panelThickness - backPanelThickness - 2, w: tGrooveW, h: backPanelThickness + 2 }, mirroredTopNotches);
  ...
  if (enableColumn) {
    ...
    if (showBackStretchers && showNailHoles) {
      const topStretcherYTop = columnPanelHeight / 2;
      const bottomStretcherYTop = -columnPanelHeight / 2 + wallBottomRecess + 100;
      
      csrHoles.push({ y: topStretcherYTop - 25, z: isBlindLeft ? -zBack : zBack, r: technicalR });
      csrHoles.push({ y: topStretcherYTop - 80, z: isBlindLeft ? -zBack : zBack, r: technicalR });
      
      csrHoles.push({ y: bottomStretcherYTop - 25, z: isBlindLeft ? -zBack : zBack, r: technicalR });
      csrHoles.push({ y: bottomStretcherYTop - 80, z: isBlindLeft ? -zBack : zBack, r: technicalR });
    }


    const grooveX = isBlindLeft ? columnDepth - panelThickness - (backPanelThickness + 2) : panelThickness;
    addPanelToZip('Column_Side_Return', columnDepth, columnPanelHeight, csrHoles, { x: grooveX, y: 0, w: backPanelThickness + 2, h: columnPanelHeight });
