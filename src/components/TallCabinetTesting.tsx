    ...
    <group position={[width / 2, innerHeight / 2 + toeKickHeight, depth / 2]}>
      ...
      {showBackPanel && shouldShow('backPanel') && (
        <mesh position={[0 + getOffset('backPanel')[0], 0 + getOffset('backPanel')[1], -innerDepth / 2 + panelThickness + backPanelThickness / 2 + getOffset('backPanel')[2]]} castShadow receiveShadow visible={!skeletonView}>
          ...
          <meshStandardMaterial color={settings.isStudio && settings.woodTexture ? '#ffffff' : showDifferentPanelColors ? panelColors.backPanel : backPanelColor} map={settings.isStudio ? settings.woodTexture : undefined} roughness={0.5} metalness={0} transparent={settings.opacity < 1} opacity={settings.opacity} />
        </mesh>
      )}
      ...
      {showBackStretchers && (
        <>
          ...
          <mesh position={[0, -innerHeight / 2 + panelThickness + settings.wallBottomRecess + 50, -innerDepth / 2 + panelThickness / 2 + getOffset('bottomStretcher')[2]]} castShadow receiveShadow visible={!skeletonView}>
            <boxGeometry args={[innerWidth - panelThickness * 2, 100, panelThickness]} />
            <meshStandardMaterial color={settings.isStudio && settings.woodTexture ? '#ffffff' : getPanelColor('bottomStretcher')} map={settings.isStudio ? settings.woodTexture : undefined} roughness={0.4} metalness={0} transparent={settings.opacity < 1} opacity={settings.opacity} />
          </mesh>
          {skeletonView && (
