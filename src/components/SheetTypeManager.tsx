...
export const SheetTypeManager: React.FC<SheetTypeManagerProps> = ({
  ...
  const loadData = async () => {
    ...
    const cachedTypes = sheetTypeService.getCachedSheetTypes();
    const cachedAccs = expenseTemplateService.getCachedTemplates();

    if (cachedTypes) setSheetTypes(cachedTypes);
    if (cachedAccs) setAccessories(cachedAccs);
    
    // Only show loading if we have absolutely nothing
    if (!cachedTypes && !cachedAccs) {
      setIsLoading(true);
    }
                    ...
                    <tr key={sheetType.id} className="group hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                      {editingId === sheetType.id ? (
                        <>
                          ...
                          <td className="px-3 py-3"><input type="number" value={editFormData.thickness} onChange={(e) => setEditFormData({ ...editFormData, thickness: Number(e.target.value) })} className="w-20 px-2 py-1 border border-slate-300 dark:border-slate-600 rounded text-sm bg-white dark:bg-slate-700 dark:text-white" /></td>
                          <td className="px-3 py-3">
                            <div className="flex items-center gap-1">
                              <input type="number" value={editFormData.width} onChange={(e) => setEditFormData({ ...editFormData, width: Number(e.target.value) })} className="w-16 px-2 py-1 border border-slate-300 dark:border-slate-600 rounded text-xs bg-white dark:bg-slate-700 dark:text-white" />
                              <span className="text-slate-400">x</span>
                              <input type="number" value={editFormData.length} onChange={(e) => setEditFormData({ ...editFormData, length: Number(e.target.value) })} className="w-16 px-2 py-1 border border-slate-300 dark:border-slate-600 rounded text-xs bg-white dark:bg-slate-700 dark:text-white" />
                            </div>
                          </td>
                          <td className="px-3 py-3"><input type="number" value={editFormData.price_per_sheet} onChange={(e) => setEditFormData({ ...editFormData, price_per_sheet: Number(e.target.value) })} className="w-24 px-2 py-1 border border-slate-300 dark:border-slate-600 rounded text-sm bg-white dark:bg-slate-700 dark:text-white" /></td>
                          <td className="px-3 py-3 text-right">
                            <div className="flex justify-end gap-1">
                              <button onClick={() => handleUpdateSheet(sheetType.id)} className="p-2 text-green-500 hover:bg-green-50 rounded-lg"><Plus size={16} /></button>
          ...
          <div className="p-0">
            {showAddForm === 'accessory' && (
              <div className="mb-4 p-4 bg-slate-50 dark:bg-slate-900 rounded-xl border-2 border-dashed border-blue-200 dark:border-blue-900/50 animate-in slide-in-from-top-4">
                <h4 className="text-xs font-black mb-4 text-blue-600 uppercase tracking-widest">New Accessory</h4>
