...
function toMeters(value, lengthUnit) {
  const factor = UNIT_TO_METERS[lengthUnit] ?? 1;
  return value * factor;
}

function getXZ(point) {
...
function pickScaleBarMeters(roomWidthMeters) {
  // Prefer “nice” bars; keep simple and predictable.
...
function cabinetFillForKind(kind) {
  // Stick to neutral grayscale so we don’t introduce a new UI theme.
  switch (kind) {
    case 'sink_base':
      return 'rgba(15, 23, 42, 0.12)'; // slate-ish
    case 'hood_unit':
...
function drawFloor(ctx, floorPointsPx) {
...
function drawWalls(ctx, walls, mapPoint, scalePxPerMeter, lengthUnit) {
  ctx.save();
  ctx.strokeStyle = '#0f172a';
  ctx.lineCap = 'butt';
  ...
  for (const wall of walls) {
    ...
    // Split wall segment into parts that exclude openings.
    const dx = bx - ax;
    const dz = bz - az;
    ...
    // Optional: mark opening centers as a thin tick (helps readability).
    ctx.lineWidth = 1;
    for (const c of merged) {
...
function drawCabinets(ctx, objects, mapPoint, lengthUnit) {
  ctx.save();
  ctx.strokeStyle = '#0f172a';
  ctx.lineWidth = 2;
...
function drawScaleBar(ctx, boundsMeters, scalePxPerMeter, canvasWidth, canvasHeight) {
  ctx.save();
...
export function renderKitchenPlanToCanvas(canvas, data, options = {}) {
  if (!canvas) return;
  const ctx = canvas.getContext('2d');
  if (!ctx) return;

  const lengthUnit = data?.units?.lengthUnit ?? data?.units?.axisConvention?.lengthUnit ?? 'm';
  const planViewPlane = data?.units?.planViewPlane ?? data?.units?.axisConvention?.planViewPlane ?? 'XZ';
  if (planViewPlane !== 'XZ') {
    // We only support XZ top-down per requirement.
    // Still render using XZ if provided differently.
  }

  const { width = 1100, height = 700, paddingPx = 40, scalePxPerMeter = 100, background = '#f8fafc' } = options;

  // Respect the canvas backing resolution for crisp PDF exports.
  ...
  // Use the provided scale factor, but if it doesn’t fit, fall back to fit-to-canvas.
  const fitScale = Math.min(
    (width - paddingPx * 2) / roomW,
    (height - paddingPx * 2) / roomH
  );
  const finalScale = Math.min(scalePxPerMeter, fitScale);

  const mapPoint = ({ x, z }) => {
    // X right, Z forward. Canvas Y grows downward, so flip Z.
...
export function exportCanvasToPDF(canvas, options = {}) {
  if (!canvas) return;
  const { jsPDF } = options;
  if (!jsPDF) throw new Error('jsPDF instance not provided');

  const filename = options.filename ?? 'kitchen-plan.pdf';
  ...
  // Convert canvas px to mm at 96dpi for aspect ratio math.
  const pxToMm = (px) => (px * 25.4) / 96;
  const imgWmm = pxToMm(canvas.width / (window.devicePixelRatio || 1));
  const imgHmm = pxToMm(canvas.height / (window.devicePixelRatio || 1));
