    ...
    return (
      <div className="fixed inset-0 bg-slate-900 flex items-center justify-center z-50">
        <div className="text-center">
          <div className="font-black text-3xl mb-4">CAB<span className="text-orange-600">ENGINE</span></div>
          ...
          >
            <div className={`w-full px-4 mb-8 flex items-center ${isSidebarExpanded ? 'justify-between' : 'justify-center'}`}>
              ...
              <button
                ...
                className="p-2 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 hover:text-orange-600 transition-all shadow-sm"
              >
                {isSidebarExpanded ? <ChevronLeft size={20} /> : <LayoutDashboard size={24} className="text-orange-600" />}
              </button>
            </div>
            ...
            <div className="mt-auto flex flex-col gap-2 w-full px-3">
              {user ? (
                <button
                  onClick={() => setShowAuthModal(true)}
                  className={`flex items-center gap-4 p-3 rounded-xl bg-slate-100 dark:bg-slate-800 text-orange-600 hover:bg-slate-200 dark:hover:bg-slate-700 transition-all w-full ${!isSidebarExpanded ? 'justify-center' : ''}`}
                  title={user.email || ''}
                >
...
const MobileNavButton = ({ active, onClick, icon, label, path, isDirty, canDiscard, onSave }: any) => {
  ...
  return (
    <button
      onClick={handleClick}
      className={`flex-1 flex flex-col items-center justify-center gap-1 ${active ? 'text-indigo-700 dark:text-orange-600' : 'text-slate-400'}`}
    >
      {icon}
